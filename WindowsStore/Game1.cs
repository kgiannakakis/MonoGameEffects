...
namespace MonoGameEffects
{
    ...
    public class Game1 : Game
    {
        ...
        protected override void LoadContent()
        {
            ...
            texturedEffect.Parameters["DiffuseLightDirection"].SetValue(new Vector3(-1, 1, 0));
        ...
        private void DrawModelWithBumpMapEffect(Model model, Matrix world, Matrix view, Matrix projection)
        {
            foreach (ModelMesh mesh in model.Meshes)
            {
                foreach (ModelMeshPart part in mesh.MeshParts)
                {
                    ...
                    bumpMapEffect.Parameters["ModelTexture"].SetValue(texture);

                    bumpMapEffect.Parameters["AmbientIntensity"].SetValue(0.1f);
                    bumpMapEffect.Parameters["DiffuseIntensity"].SetValue(1f);
                    bumpMapEffect.Parameters["SpecularIntensity"].SetValue(1f);
                    ...
                    bumpMapEffect.Parameters["NormalMap"].SetValue(normalMap);
                }
                mesh.Draw();
            }
        }
